...
async function executeCustomQuery() {
    const query = prompt(
        'Ingresa una consulta SQL (solo SELECT):\n\n' +
        'Ejemplos:\n' +
    ...
    showLoading('results', 'Ejecutando consulta...');
    
    try {
        const data = await api.query.execute(query);
        
        if (data.success && data.data.length > 0) {
            const columns = Object.keys(data.data[0]);
            const html = `
                <div style="margin-bottom: 10px; padding: 10px; background: #166534; border-radius: 4px; border: 1px solid #22c55e;">
                ...
                <pre style="background: #0f172a; color: #22c55e; padding: 10px; border-radius: 4px; font-size: 12px; border: 1px solid #334155;">${escapeHtml(query)}</pre>
                <table>
                    <thead>
            ...
            document.getElementById('results').innerHTML = `
                <div style="padding: 40px; text-align: center; color: #94a3b8;">
                    ...
                    <pre style="background: #0f172a; color: #22c55e; padding: 10px; border-radius: 4px; font-size: 12px; text-align: left; border: 1px solid #334155;">${escapeHtml(query)}</pre>
                </div>
            `;
        }
    } catch (error) {
